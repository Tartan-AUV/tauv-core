...
#define WATCHDOG_INTEREST_FAULT_MASK                                             \
    (ESC_FAULT_SIGNAL_LOSS | ESC_FAULT_ERROR_ACTIVE | ESC_FAULT_OUTPUT_SHORTED | \
     ESC_FAULT_STARTUP_CHECK_FAILED)
...
Watchdog::Watchdog(std::string prefix)
    ...
    const auto ensure_positive_param =
        [this](const char* name, const double value, const double fallback) {
            if (value > 0.0) {
                return value;
            }
    ...
    const auto ensure_non_negative_param =
        [this](const char* name, const double value, const double fallback) {
            if (value >= 0.0) {
                return value;
            }
    ...
    esc_topic_ = this->declare_parameter<std::string>(WATCHDOG_ESC_TOPIC_PARAM,
                                                      WATCHDOG_ESC_TELEMETRY_TOPIC);
    imu_topic_ = this->declare_parameter<std::string>(WATCHDOG_IMU_TOPIC_PARAM,
                                                      prefix_ + WATCHDOG_IMU_TOPIC_SUFFIX);
    system_state_topic_ = this->declare_parameter<std::string>(WATCHDOG_SYSTEM_STATE_TOPIC_PARAM,
                                                               WATCHDOG_SYSTEM_STATE_TOPIC);
    heartbeat_check_hz_ =
        ensure_positive_param(WATCHDOG_HEARTBEAT_FREQUENCY_PARAM,
                              this->declare_parameter<double>(WATCHDOG_HEARTBEAT_FREQUENCY_PARAM,
                                                              WATCHDOG_HEARTBEAT_CHECK_HZ),
                              WATCHDOG_HEARTBEAT_CHECK_HZ);
    esc_timeout_s_ =
        ensure_positive_param(WATCHDOG_ESC_TIMEOUT_PARAM,
                              this->declare_parameter<double>(WATCHDOG_ESC_TIMEOUT_PARAM,
                                                              WATCHDOG_ESC_TIMEOUT_S),
                              WATCHDOG_ESC_TIMEOUT_S);
    stale_startup_grace_s_ =
        ensure_non_negative_param(WATCHDOG_STALE_STARTUP_GRACE_PARAM,
                                  this->declare_parameter<
                                      double>(WATCHDOG_STALE_STARTUP_GRACE_PARAM,
                                              WATCHDOG_STALE_STARTUP_GRACE_S),
                                  WATCHDOG_STALE_STARTUP_GRACE_S);
    warning_temperature_c_ = this->declare_parameter<double>(WATCHDOG_WARNING_TEMPERATURE_PARAM,
                                                             WATCHDOG_WARNING_TEMPERATURE_C);
    error_temperature_c_ = this->declare_parameter<double>(WATCHDOG_ERROR_TEMPERATURE_PARAM,
                                                           WATCHDOG_ERROR_TEMPERATURE_C);
    error_voltage_v_ =
        this->declare_parameter<double>(WATCHDOG_ERROR_VOLTAGE_PARAM, WATCHDOG_ERROR_VOLTAGE_V);
    roll_threshold_deg_ =
        ensure_non_negative_param(WATCHDOG_ROLL_THRESHOLD_PARAM,
                                  this->declare_parameter<double>(WATCHDOG_ROLL_THRESHOLD_PARAM,
                                                                  WATCHDOG_ERROR_ROLL_DEG),
                                  WATCHDOG_ERROR_ROLL_DEG);
    pitch_threshold_deg_ =
        ensure_non_negative_param(WATCHDOG_PITCH_THRESHOLD_PARAM,
                                  this->declare_parameter<double>(WATCHDOG_PITCH_THRESHOLD_PARAM,
                                                                  WATCHDOG_ERROR_PITCH_DEG),
                                  WATCHDOG_ERROR_PITCH_DEG);
    angular_velocity_threshold_radps_ =
        ensure_non_negative_param(WATCHDOG_ANGULAR_VELOCITY_THRESHOLD_PARAM,
                                  this->declare_parameter<
                                      double>(WATCHDOG_ANGULAR_VELOCITY_THRESHOLD_PARAM,
                                              WATCHDOG_ERROR_ANGULAR_VELOCITY_RADPS),
                                  WATCHDOG_ERROR_ANGULAR_VELOCITY_RADPS);
    mission_timeout_s_ =
        this->declare_parameter<double>(WATCHDOG_MISSION_TIMEOUT_PARAM, WATCHDOG_MISSION_TIMEOUT_S);

    const std::vector<int64_t> default_expected_esc_ids = WATCHDOG_EXPECTED_ESC_IDS;
    const auto expected_esc_ids_param =
        this->declare_parameter<std::vector<int64_t>>(WATCHDOG_EXPECTED_ESC_IDS_PARAM,
    ...
    esc_sub_ = this->create_subscription<
        tauv_msgs::msg::EscTelemetry>(esc_topic_,
                                      rclcpp::SensorDataQoS(),
                                      std::bind(&Watchdog::escTelemetryCallback,
                                                this,
                                                std::placeholders::_1));

    imu_sub_ = this->create_subscription<sensor_msgs::msg::Imu>(imu_topic_,
                                                                rclcpp::SensorDataQoS(),
                                                                std::bind(&Watchdog::imuCallback,
                                                                          this,
                                                                          std::placeholders::_1));
    ...
    heartbeat_timer_ =
        this->create_wall_timer(timer_period, std::bind(&Watchdog::heartbeatCheckCallback, this));

    publishSystemState();

    RCLCPP_INFO(this->get_logger(),
                "Watchdog started. esc_topic=%s imu_topic=%s state_topic=%s timeout=%.2fs "
                "check_hz=%.2f expected_esc_count=%zu",
                esc_topic_.c_str(),
                imu_topic_.c_str(),
...
void Watchdog::imuCallback(const sensor_msgs::msg::Imu::SharedPtr msg) {
    ...
    const double attitude_norm_deg = std::sqrt(roll_deg * roll_deg + pitch_deg * pitch_deg);
    const bool attitude_exceeds_threshold = attitude_norm_deg > roll_threshold_deg_;
    ...
    if (attitude_state_changed && imu_attitude_fault_) {
        RCLCPP_ERROR(this->get_logger(),
                     "IMU attitude limit exceeded: norm=%.2f deg (limit=%.2f), |roll|=%.2f deg, "
                     "|pitch|=%.2f deg",
                     attitude_norm_deg,
                     roll_threshold_deg_,
                     roll_deg,
    ...
    if (attitude_state_changed && !imu_attitude_fault_) {
        RCLCPP_INFO(this->get_logger(),
                    "IMU attitude returned within limits: norm=%.2f deg, |roll|=%.2f deg, "
                    "|pitch|=%.2f deg",
                    attitude_norm_deg,
                    roll_deg,
                    pitch_deg);
    ...
    const bool angular_velocity_state_changed =
        angular_velocity_exceeds_threshold != imu_angular_velocity_fault_;
    ...
    if (angular_velocity_state_changed && imu_angular_velocity_fault_) {
        RCLCPP_ERROR(this->get_logger(),
                     "IMU angular velocity limit exceeded: |wx|=%.2f rad/s, |wy|=%.2f rad/s, "
                     "|wz|=%.2f rad/s (limit=%.2f)",
                     omega_x_radps,
                     omega_y_radps,
                     omega_z_radps,
                     angular_velocity_threshold_radps_);
    ...
    if (angular_velocity_state_changed && !imu_angular_velocity_fault_) {
        RCLCPP_INFO(this->get_logger(),
                    "IMU angular velocity returned within limits: |wx|=%.2f rad/s, |wy|=%.2f "
                    "rad/s, |wz|=%.2f rad/s",
                    omega_x_radps,
                    omega_y_radps,
                    omega_z_radps);
...
void Watchdog::heartbeatCheckCallback() {
    ...
    if ((now - startup_time_).seconds() >= stale_startup_grace_s_) {
        for (const uint8_t esc_id : monitoredEscIds()) {
            ...
            const bool stale = it == last_esc_telemetry_time_.end() ||
                               (now - it->second).seconds() > esc_timeout_s_;

            // log error if this is first time we have seen it stale, but do not log again until it
            // recovers and goes stale again.
...
void Watchdog::publishSystemState() {
    const bool has_error_now = !escs_with_cared_faults_.empty() || !stale_esc_ids_.empty() ||
                               imu_attitude_fault_ || imu_angular_velocity_fault_;

    const bool previous_state = system_in_error_;
    system_in_error_ = has_error_now || mission_timed_out_;
    ...
    if (state_changed) {
        RCLCPP_INFO(this->get_logger(), "Watchdog system state changed: %s", msg.data.c_str());
    }
}
